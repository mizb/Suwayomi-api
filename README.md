...
配套 API 交付版本为 `2026.07.17.8`。升级 APK 不要求改变 API JSON，但建议同时重建 API 容器以取得每周推荐字母型 type ID、数字型章节/album/列表 ID 兼容、非法布尔/浮点 ID 拒绝、严格列表总数、失败传播、请求预算、单图内预取字节硬边界、健康排序 A→B→C→D→E 最多三轮的瞬态故障恢复、列表/album 缓存、CDN failover 和可信代理修复：
